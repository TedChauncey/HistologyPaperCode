...
c = np.round(c*100,10, out = None) # convert decimals into larger numbers. remove this shortly!!!
...
stat = stats.kruskal(Preds_forADC, Preds_forSCC, Preds_forOther)
print("ADC vs SCC vs Other pval:", stat[1])

stat = stats.kruskal(Preds_forADC, Preds_forSCC)
print("ADC vs SCC pval:", stat[1])

stat = stats.kruskal(Preds_forADC, Preds_forOther)
print("ADC vs Other pval:", stat[1])

stat = stats.kruskal(Preds_forSCC, Preds_forOther)
print("SCC vs Other pval:", stat[1])

data = [Preds_forADC, Preds_forSCC, Preds_forOther]
plt.figure()
plt.boxplot(data, 0, '')
plt.xticks([1, 2, 3], ['ADC', 'SCC', 'Other'])
plt.ylabel("Prediction probability")
plt.title ('CNN biomarker 1 predictions by histology - BLCS')
plt.text(1, 20, r'Kruskal-Wallis H-test: p=0.05', fontsize=8, color = 'blue')
plt.text(1, 18, r'ADC v SCC: p=0.02', fontsize=8, color = 'red')
plt.text(1, 16, r'ADC v Other: p=0.60', fontsize=8, color = 'red')
plt.text(1, 14, r'SCC v Other: p=0.08', fontsize=8, color = 'red')
#plt.boxplot( data, 1, '')
plt.show()
